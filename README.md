...
### Ερώτημα 2 (Αποθήκευση δεδομένων σε βάση δεδομένων).
...
#### 2. β.
- Συνδέσαμε τους αισθητήρες φύλλα στον υπολογιστή και τους φορτώσαμε με τον κώδικα του αρχείου `Ερώτημα_1_2_b_Leaf.c`.
- Αντίστοιχα φορτώσαμε στο TelosB τον κώδικα του αρχείου `Ερώτημα_1_2_b_Telosb.c`.
- Κρατήσαμε το TelosB συνδεδεμένο στο PC και τοποθετήσαμε τα φύλλα σε απόσταση τροφοδοτούμενα από μπαταρίες.
- χρησιμοποιήθηκε το εργαλείο screen για την προβολή των μηνυμάτων του TelosB και επιβεβαιώθηκε η σωστή λήψη χωρίς παρεμβολές.

#### 2. γ.
- Κάναμε λήψη του terminator, ώστε να το τρέχουμε παράλληλα με το terminal και να βλέπουμε in real time τις εγγραφές μας στη βάση.
- Κατεβάσαμε το mysql connector για να επικοινωνεί η βάση μας με το πρόγραμμα που θα παίρνει τα δεδομένα και θα τα καταγράφει
- Το πρόγραμμα το ονομάσαμε `out_pipe.py` και τοποθετήθηκε στο `test_nullnet directory`.
- Κάναμε select * from measurements στη βάση μας και είδαμε ότι κάθε 20 δευτερόλεπτα γίνονται επιτυχώς οι εγγραφές.

### Ερώτημα 3 (Οπτικοποίηση δεδομένων).
- Κατεβάσαμε το `Grafana` στο Ubuntu
- Συνδέσαμε τη βάση με τον σέρβερ του Grafana.
- Στο repo έχουμε ανεβάσει δύο screenshots που αποτυπώνουν την real time προβολή δεδομένων και το ιστορικό. `history.png` και `real-time.png`
